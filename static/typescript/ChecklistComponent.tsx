...
> {
  ...
  public render() {
    ...
    return (
      <section className={this.completed() ? "completed" : ""}>
        <link
                  ...
                  <div className="checkedby">
                    {item.CheckedBy.map((user) => {
                      return (
                        <span
                          className="user"
                          key={`item-${item.Number}-checkedby-${user.ID}`}
                        >
  ...
  private navigateToStage(stage: string) {
    ...
    if (stage === "") {
      location.replace(
        `/${checklistRef.Owner}/${checklistRef.Repo}/pull/${checklistRef.Number}`
      );
    } else {
      location.replace(
        `/${checklistRef.Owner}/${checklistRef.Repo}/pull/${checklistRef.Number}/${stage}`
      );
    }
  }

  private handleOnClickChecklistItem = (
    item: API.ChecklistItem
